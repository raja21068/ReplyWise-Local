...
function extractExternalMessageIdFromMetadata(metadata) {
...
function normalizeOutgoingRow(q, contact) {
  return {
    ...q,
    display_name: contact?.display_name,
    contact: normalizeContactRow(contact || {}),
    externalContactId: q.external_contact_id,
    displayName: contact?.display_name,
  };
}

function validateOutgoingForSend(q, contact, requestedChannel = null) {
  const reasons = [];
  const channel = normalizeChannel(q?.channel || contact?.channel);
  if (requestedChannel && channel !== normalizeChannel(requestedChannel)) reasons.push(`channel mismatch: ${channel} vs ${normalizeChannel(requestedChannel)}`);
  if (!contact?.id) reasons.push('contact not found');
  if (!q?.body || !String(q.body).trim()) reasons.push('empty body');
  if (!q?.external_contact_id && !contact?.external_contact_id && !q?.whatsapp_id) reasons.push('missing destination id');
  if (contact?.id && q?.contact_id && contact.id !== q.contact_id) reasons.push('contact id mismatch');
  const rules = contact?.contact_rules || {};
  if (contact?.blocked || rules.blocked) reasons.push('contact is blocked');
  if (channel === 'whatsapp') {
    const to = String(q.external_contact_id || contact?.external_contact_id || q.whatsapp_id || '').trim();
    if (!to) reasons.push('missing WhatsApp chat id');
    if (to.includes(',')) reasons.push('refusing multiple WhatsApp recipients');
    if (/\s/.test(to)) reasons.push('WhatsApp chat id contains spaces');
  }
...
async function insertMessage({ contactId, direction, body, timestamp, metadata, media_type, media_summary, external_message_id }) {
  const store = await readStore();
  const row = {
    id: uuidv4(),
    ...
    external_message_id: external_message_id || extractExternalMessageIdFromMetadata(metadata),
...
async function findMessageByExternalMessageId({ contactId, externalMessageId }) {
  if (!contactId || !externalMessageId) return null;
  const wanted = String(externalMessageId);
  const store = await readStore();
  const row = store.messages.find(m => {
    if (m.contact_id !== contactId) return false;
    if (String(m.external_message_id || '') === wanted) return true;
    const metaId = extractExternalMessageIdFromMetadata(m.metadata);
    return String(metaId || '') === wanted;
  });
  return normalizeMessageRow(row);
}

async function getSuggestionByIncomingMessageId(incomingMessageId) {
  if (!incomingMessageId) return null;
  const store = await readStore();
  const s = store.suggestions.find(x => x.incoming_message_id === incomingMessageId);
  if (!s) return null;
...
async function getPendingOutgoing({ channel = null, limit = 10 } = {}) {
  ...
  return store.outgoing_queue
    ...
    .map(q => {
      ...
      return normalizeOutgoingRow(q, c);
    });
}

async function claimPendingOutgoing({ channel = null, limit = 10, bridge = null, leaseSeconds = 90 } = {}) {
  const store = await readStore();
  const ch = channel ? normalizeChannel(channel) : null;
  const ts = now();
  const cutoff = Date.now() - Math.max(15, Number(leaseSeconds) || 90) * 1000;
  const candidates = store.outgoing_queue
    .filter(q => {
      if (ch && normalizeChannel(q.channel) !== ch) return false;
      if (q.status === 'pending') return true;
      if (q.status === 'sending' && q.claimed_at) {
        const claimedAt = new Date(q.claimed_at).getTime();
  ...
  for (const q of candidates) {
    const contact = store.contacts.find(x => x.id === q.contact_id) || null;
    const validation = validateOutgoingForSend(q, contact, ch);
    if (!validation.ok) {
      q.status = 'failed';
      q.error_log = `Sender guard blocked: ${validation.reasons.join('; ')}`.slice(0, 500);
      q.failed_at = ts;
      q.updated_at = ts;
      continue;
    }
    ...
    q.claimed_at = ts;
    q.updated_at = ts;
    if (bridge) q.claimed_by = String(bridge).slice(0, 120);
    claimed.push(normalizeOutgoingRow(q, contact));
  }

  if (candidates.length) await writeStore(store);
  return claimed;
}
...
module.exports = {
  ...
  getSuggestionByIncomingMessageId,
  createSuggestion,
  getSuggestionById,
