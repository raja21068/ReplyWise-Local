...
app.get('/', async (req, res, next) => {
  try {
    const state = await buildDashboardState(req.query.contactId, req.query.view);
...
app.get('/api/dashboard/state', async (req, res, next) => {
  try {
    res.json({ ok: true, ...(await buildDashboardState(req.query.contactId, req.query.view)) });
  } catch (err) { next(err); }
});
...
// Lightweight non-message WhatsApp events from browser agents.
// These prevent agent-side failures for reaction/revoked callbacks and keep them visible in Logs.
app.post('/api/ingest/:channel/revoked', async (req, res, next) => {
  try {
    const channel = db.assertSupportedChannel(req.params.channel);
    await db.addSystemEvent?.({
      type: 'revoked', status: 'warning', channel,
      externalContactId: req.body.from || req.body.externalContactId || null,
      title: 'Message revoked',
      detail: req.body.wa_msg_id ? `Revoked WhatsApp message ${req.body.wa_msg_id}` : 'A message was revoked',
      meta: req.body || {},
    });
    res.json({ ok: true });
  } catch (err) { next(err); }
...
app.post('/api/ingest/:channel/reaction', async (req, res, next) => {
  try {
    const channel = db.assertSupportedChannel(req.params.channel);
    await db.addSystemEvent?.({
      type: 'reaction', status: 'ok', channel,
      externalContactId: req.body.from || req.body.senderId || null,
      title: `Reaction ${req.body.emoji || ''}`.trim(),
      detail: req.body.wa_msg_id ? `Reaction on ${req.body.wa_msg_id}` : 'Reaction event received',
      meta: req.body || {},
    });
    res.json({ ok: true });
  } catch (err) { next(err); }
...
app.post('/api/ingest/:channel', async (req, res, next) => {
  try {
    const result = await processIncomingMessageQueued({
      ...
      metadata: { source: `${req.params.channel}-browser-agent`, wa_msg_id: req.body.wa_msg_id || req.body.message_id || null, raw: req.body },
    });
...
app.get('/api/bridge/pending-outgoing', async (req, res, next) => {
  try {
    // v7: claim rows before returning them so the browser agent cannot send
    // the same queue item twice when polling overlaps or WhatsApp is slow.
    const rows = typeof db.claimPendingOutgoing === 'function'
      ? await db.claimPendingOutgoing({
          channel: req.query.channel || null,
          limit: Number(req.query.limit || 5),
          bridge: req.query.bridge || req.query.channel || 'browser-agent',
          leaseSeconds: Number(req.query.leaseSeconds || 90),
        })
      : await db.getPendingOutgoing({ channel: req.query.channel || null, limit: Number(req.query.limit || 5) });
...
app.post('/api/bridge/outgoing/:id/ack', async (req, res, next) => {
  try {
    await db.addSystemEvent?.({
      type: 'ack',
      status: 'ok',
      channel: 'whatsapp',
      title: `WhatsApp ACK ${req.body.ack_label || req.body.ack || ''}`.trim(),
      detail: req.body.wa_msg_id ? `Message ${req.body.wa_msg_id}` : `Queue item ${req.params.id}`,
      meta: { queueId: req.params.id, ...req.body },
    });
    res.json({ ok: true });
  } catch (err) { next(err); }
});
...
// flips status back to 'pending' when the time arrives — agents pick it up
...
function extractIncomingExternalMessageId(metadata = {}) {
  if (!metadata || typeof metadata !== 'object') return null;
  return metadata.external_message_id
    || metadata.wa_msg_id
    || metadata.message_id
    || metadata.id
    || metadata.raw?.wa_msg_id
    || metadata.raw?.message_id
    || metadata.raw?.id
    || null;
}

function resultFromExistingSuggestion(existingSuggestion) {
  return {
    decision: safeJson(existingSuggestion?.decisionJson || existingSuggestion?.decision_json, { action: 'duplicate', confidence: 100, reason: 'Duplicate provider message id already processed.' }),
    options: safeJson(existingSuggestion?.optionsJson || existingSuggestion?.options_json, []),
    stats: safeJson(existingSuggestion?.statsJson || existingSuggestion?.stats_json, {}),
    automation: safeJson(existingSuggestion?.automationJson || existingSuggestion?.automation_json, {}),
    provider: 'idempotent-cache',
  };
}

function groupShouldStaySilent({ is_group, mentioned_me, reply_to_me }) {
  return Boolean(is_group) && !Boolean(mentioned_me) && !Boolean(reply_to_me);
}

function buildNoReplyResult(reason) {
  return {
    provider: 'nano-safety-bot',
    decision: { action: 'no', confidence: 99, risk_level: 'low', reason, best_move: 'Stay silent unless you are mentioned or directly replied to.', avoid: 'Do not send automatic replies in groups without mention.' },
    options: [{ action: 'skip', tone: 'safety', text: '[No reply — safety bot blocked auto response]', score: 100, risk: 'low', rationale: reason }],
    stats: { warmthScore: 0, incomingAvgWords: 0, outgoingAvgWords: 0 },
    automation: {
      version: 'nano-safety-v7',
      mode: 'manual',
      auto_choose: { enabled: false, allowed: false },
      auto_send: { enabled: false, allowed: false, blocked_reasons: [reason], whitelist_only: true, whitelisted: false },
      recommended_index: null,
      recommended_tone: null,
      recommended_text: null,
      safety_class: 'blocked-group-no-mention',
    },
  };
}
...
async function processIncomingMessage({
  ...
  const externalMessageId = extractIncomingExternalMessageId(metadata);
  if (externalMessageId && typeof db.findMessageByExternalMessageId === 'function') {
    const existingIncoming = await db.findMessageByExternalMessageId({ contactId: contact.id, externalMessageId });
    if (existingIncoming) {
      const existingSuggestion = await db.getSuggestionByIncomingMessageId(existingIncoming.id);
      await db.addSystemEvent?.({
        type: 'dedupe', status: 'warning', channel: normalizedChannel, contactId: contact.id, externalContactId,
        title: 'Duplicate WhatsApp message ignored',
        detail: `Provider message id ${externalMessageId} was already processed.`,
        meta: { externalMessageId },
      });
      return {
        contact,
        incoming: existingIncoming,
        suggestion: existingSuggestion || { id: null, status: 'duplicate' },
        result: resultFromExistingSuggestion(existingSuggestion),
        autoSent: false,
        duplicate: true,
      };
    }
  ...
  if (groupShouldStaySilent({ is_group, mentioned_me, reply_to_me })) {
    const reason = 'Group message did not mention or directly reply to you.';
    await db.setProcessingStatus?.({ key: processingKey, channel: normalizedChannel, contactId: contact.id, externalContactId, stage: 'safety', status: 'processing', detail: reason });
    const result = buildNoReplyResult(reason);
    const suggestion = await db.createSuggestion({ contactId: contact.id, incomingMessageId: incoming.id, result });
    await db.skipSuggestion(suggestion.id);
    await db.addSystemEvent?.({ type: 'safety', status: 'warning', channel: normalizedChannel, contactId: contact.id, externalContactId, title: 'Safety Bot blocked group auto-reply', detail: reason, meta: { suggestionId: suggestion.id } });
    bus.emit('suggestion.created', {
      suggestionId: suggestion.id, contactId: contact.id, contactName: contact.displayName || contact.display_name || externalContactId,
      channel: normalizedChannel, incomingBody: String(workingBody).slice(0, 200), decisionAction: 'no', confidence: 99, autoSent: false, mediaType: incoming.media_type, timestamp: new Date().toISOString(),
    });
    return { contact, incoming, suggestion: { ...suggestion, status: 'skipped' }, result, autoSent: false };
  }
  ...
  } else if (result.provider_route?.attempts?.length && result.provider_route?.used === 'local') {
    await db.addSystemEvent?.({
      type: 'ai', status: 'warning', channel: normalizedChannel, contactId: contact.id, externalContactId,
      title: 'Cloud AI failed; local Nano Bot used',
      detail: result.provider_route.attempts.map(a => `${a.provider}: ${a.error}`).join(' | ').slice(0, 900),
      meta: result.provider_route,
    });
  }
...
async function buildDashboardState(selectedContactId = null, view = 'all') {
  ...
  const pendingContactIds = new Set(pendingSuggestions.map(s => s.contactId || s.contact_id).filter(Boolean));
  const hasMessageFlag = (contactId, flag) => allMessages.some(m => {
    if (m.contact_id !== contactId) return false;
    const meta = m.metadata || {};
    return Boolean(m[flag] || meta[flag]);
  });
  const isUnread = (contact) => pendingContactIds.has(contact.id) || allMessages.some(m => m.contact_id === contact.id && m.direction === 'incoming' && (m.metadata || {}).unread);
  const isGroup = (contact) => Boolean(contact.is_group || contact.contact_rules?.is_group || contact.contactRules?.is_group || hasMessageFlag(contact.id, 'is_group'));
  const isMention = (contact) => Boolean(hasMessageFlag(contact.id, 'mentioned_me') || hasMessageFlag(contact.id, 'reply_to_me'));
  const isStarred = (contact) => Boolean(contact.is_starred || contact.contact_rules?.is_starred || contact.contactRules?.is_starred || hasMessageFlag(contact.id, 'is_starred'));
  const isBlocked = (contact) => Boolean(contact.blocked || contact.contact_rules?.blocked || contact.contactRules?.blocked);
  const isLead = (contact) => {
    const stage = String(contact.conversationStage || contact.conversation_stage || '').toLowerCase();
    return ['lead','new','initial','negotiation','proposal','customer','vip'].some(x => stage.includes(x)) || Boolean(contact.stats?.warmthScore || contact.stats?.leadScore);
  };
  const navCounts = {
    all: contacts.length,
    unread: contacts.filter(isUnread).length,
    groups: contacts.filter(isGroup).length,
    mentions: contacts.filter(isMention).length,
    starred: contacts.filter(isStarred).length,
    blocked: contacts.filter(isBlocked).length,
    agents: agentStatuses.length,
    auto_rules: contacts.filter(c => (c.contactRules || c.contact_rules || {}).auto_reply_enabled).length,
    smart_routing: pendingSuggestions.length + processingStatuses.filter(p => p.status === 'processing').length,
    scheduled: outgoingQueue.filter(q => q.status === 'scheduled' || q.scheduled_for || q.status === 'sending').length,
    approvals: pendingSuggestions.length,
    leads: contacts.filter(isLead).length,
    analytics: (store?.messages || []).length,
    memory: contacts.filter(c => c.profileSummary || c.profile_summary).length,
    settings: 0,
    logs: systemEvents.length,
    billing: costSummary.localActions || 0,
  };
  const viewKey = String(view || 'all').replace(/[^a-z0-9_]/gi, '') || 'all';
  ...
  const filteredContacts = contacts.filter(conversationFilter);
  const selected = selectedContactId || pendingSuggestions[0]?.contactId || filteredContacts[0]?.id || contacts[0]?.id || null;
...
function renderDashboard({
  contacts = [], allContacts = [], pendingSuggestions = [], outgoingQueue = [], agentStatuses = [], costSummary = {}, providerStatus = null,
  env = {}, selectedContactId = null, selectedContact = null, selectedMessages = [], selectedSuggestion = null,
  systemEvents = [], processingStatuses = [], weekStats = {}, view = 'all', navCounts = {},
}) {
  ...
  const currentView = String(view || 'all');
  const viewLabels = {
    all: 'All Chats', unread: 'Unread', groups: 'Groups', mentions: 'Mentions', starred: 'Starred', blocked: 'Blocked',
    agents: 'AI Agents', auto_rules: 'Auto Reply Rules', smart_routing: 'Smart Routing', scheduled: 'Scheduled', approvals: 'Manual Approval',
    leads: 'Leads', analytics: 'Analytics', memory: 'Contact Memory', settings: 'Settings', logs: 'Logs', billing: 'Billing & Usage'
  };
  const navHref = (key) => `/?view=${encodeURIComponent(key)}`;
  const navItem = (key, icon, label, count = null) => `<a class="${currentView === key ? 'active' : ''}" href="${navHref(key)}"><span>${icon}</span><span>${esc(label)}</span>${count !== null ? `<b style="margin-left:auto">${esc(count)}</b>` : ''}</a>`;
  ...
  const conversationList = contacts.slice(0, 40).map((c, index) => {
    ...
    return `<a class="conversation ${activeClass}" href="/?view=${encodeURIComponent(currentView)}&contactId=${encodeURIComponent(c.id)}">
  ...
  const sidebarNav = `<nav class="nav">
    ${navItem('all', '💬', 'All Chats', navCounts.all || 0)}
    ${navItem('unread', '●', 'Unread', navCounts.unread || 0)}
    ${navItem('groups', '👥', 'Groups', navCounts.groups || 0)}
    ${navItem('mentions', '@', 'Mentions', navCounts.mentions || 0)}
    ${navItem('starred', '★', 'Starred', navCounts.starred || 0)}
    ${navItem('blocked', '⊘', 'Blocked', navCounts.blocked || 0)}
    <div class="section-label nav-heading">Automation</div>
    ${navItem('agents', '🤖', 'AI Agents', navCounts.agents || 0)}
    ${navItem('auto_rules', '⚙', 'Auto Reply Rules', navCounts.auto_rules || 0)}
    ${navItem('smart_routing', '↪', 'Smart Routing', navCounts.smart_routing || 0)}
    ${navItem('scheduled', '⏰', 'Scheduled', navCounts.scheduled || 0)}
    ${navItem('approvals', '☑', 'Manual Approval', navCounts.approvals || 0)}
    <div class="section-label nav-heading">Intelligence</div>
    ${navItem('leads', '🎯', 'Leads', navCounts.leads || 0)}
    ${navItem('analytics', '▥', 'Analytics', navCounts.analytics || 0)}
    ${navItem('memory', '🧠', 'Contact Memory', navCounts.memory || 0)}
    <div class="section-label nav-heading">System</div>
    ${navItem('settings', '⚙', 'Settings')}
    ${navItem('logs', '▣', 'Logs', navCounts.logs || 0)}
    ${navItem('billing', '💳', 'Billing & Usage', navCounts.billing || 0)}
  </nav>`;
  ...
  const allContactRows = (allContacts.length ? allContacts : contacts);
  const pendingRows = pendingSuggestions.slice(0, 8).map(s => `<div class="queue-line"><span>☑</span><div><strong>${esc(s.display_name || s.contact?.displayName || 'Contact')}</strong><div class="sub">${esc((s.incoming_body || s.incomingMessage?.body || '').slice(0, 120))}</div></div></div>`).join('') || '<div class="empty small">No pending approvals.</div>';
  const leadRows = allContactRows.slice(0, 8).map(c => {
    const st = c.stats || {};
    const score = st.leadScore || st.warmthScore || c.lead_score || 0;
    return `<div class="queue-line"><span>🎯</span><div><strong>${esc(c.displayName || c.display_name || c.externalContactId)}</strong><div class="sub">Stage: ${esc(c.conversationStage || c.conversation_stage || 'initial')} · Score: ${esc(score)}/100</div></div></div>`;
  }).join('') || '<div class="empty small">No leads yet.</div>';
  const memoryRows = allContactRows.slice(0, 6).map(c => `<div class="queue-line"><span>🧠</span><div><strong>${esc(c.displayName || c.display_name || c.externalContactId)}</strong><div class="sub">${esc(c.profileSummary || c.profile_summary || 'Memory not generated yet.').slice(0, 160)}</div></div></div>`).join('') || '<div class="empty small">No contact memory yet.</div>';
  const settingsHtml = `<div class="mini-card"><strong>AI Provider:</strong> ${esc(env.aiProvider || 'local')}<br><strong>Agents:</strong> ${esc(env.enabledAgents || '')}<br><strong>Dry run:</strong> ${env.dryRun ? 'on' : 'off'} · <strong>Auto send:</strong> ${env.autoSend ? 'on' : 'off'} · <strong>Screenshots:</strong> ${env.screenshots ? 'on' : 'off'}</div><form method="POST" action="/api/ai/test"><button class="ghost" type="submit">Test AI backend</button></form>`;
  const focusCards = {
    agents: `<div class="card focus"><h3>AI Agents</h3>${agentsHtml}<div class="sub">Live bridge health from WhatsApp / Telegram / WeChat agents.</div></div>`,
    auto_rules: `<div class="card focus"><h3>Auto Reply Rules</h3>${activeRulesForm}</div>`,
    smart_routing: `<div class="card focus"><h3>Smart Routing</h3>${pipelineHtml}<div class="sub">Routes each message through triage, memory, reply generation, safety, then sender queue.</div></div>`,
    scheduled: `<div class="card focus"><h3>Scheduled</h3>${queueHtml}</div>`,
    approvals: `<div class="card focus"><h3>Manual Approval</h3>${pendingRows}</div>`,
    leads: `<div class="card focus"><h3>Leads</h3>${leadRows}</div>`,
    analytics: `<div class="card focus"><h3>Analytics</h3><div class="stats-grid"><div class="stat"><strong>${esc(weekStats.messages || 0)}</strong><span>Messages this week</span></div><div class="stat"><strong>${esc(weekStats.aiHandledPct || 0)}%</strong><span>AI handled</span></div><div class="stat"><strong>${esc(weekStats.sent || 0)}</strong><span>Sent</span></div><div class="stat"><strong>${esc(weekStats.errors || 0)}</strong><span>Errors</span></div></div></div>`,
    memory: `<div class="card focus"><h3>Contact Memory</h3>${memoryRows}<form method="POST" action="/api/profile-refresh/run"><button class="ghost" type="submit">Refresh memory profiles</button></form></div>`,
    settings: `<div class="card focus"><h3>Settings</h3>${settingsHtml}</div>`,
    logs: `<div class="card focus"><h3>Logs</h3>${eventsHtml}${errors.length ? '<h3 style="margin-top:14px">Recent Errors</h3>' + errorsHtml : ''}</div>`,
    billing: `<div class="card focus"><h3>Billing & Usage</h3><div class="stats-grid"><div class="stat"><strong>$${Number(costSummary.estimatedCostUsd || 0).toFixed(2)}</strong><span>Est. cost today</span></div><div class="stat"><strong>${esc(costSummary.localActions || 0)}</strong><span>Local actions</span></div><div class="stat"><strong>${esc(costSummary.cloudAiCalls || 0)}</strong><span>Cloud AI calls</span></div><div class="stat"><strong>${esc(costSummary.screenshotsUsed || 0)}</strong><span>Screenshots</span></div></div></div>`,
  };
  const focusCard = focusCards[currentView] || '';
...
module.exports = { app, processIncomingMessage, processIncomingMessageQueued, runProfileRefreshJob };
